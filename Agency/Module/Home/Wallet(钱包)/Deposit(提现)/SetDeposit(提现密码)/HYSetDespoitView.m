...
- (void)setWithViewModel:(HYSetDepositViewModel *)viewModel{
    ...
    self.removeBankCard = viewModel.removeBankCard;
    _titleLabel.text =  _removeBankCard ? @"请输入提现密码" : @"请设置提现密码";
    ...
    [[self.confirmBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        ...
        if (_removeBankCard) {
            /// 解除银行卡绑定时输入的提现密码
            NSString* pwd = self.viewModel.depositPwd;//_authCodeView.passwordString;
            !_infoCB ? : _infoCB(pwd);
            
//            [self.viewController.navigationController popViewControllerAnimated:YES];
//            [JRToast showWithText:@"解绑" duration:2];
        } else {
