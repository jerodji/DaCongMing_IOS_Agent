...
@interface JJTailorImgVC ()<UINavigationControllerDelegate,UIGestureRecognizerDelegate>
@property (nonatomic, assign) JITailorForm  tailorForm;
...
- (void)tailorImage:(UIImage*)image form:(JITailorForm)form finishCB:(FINISH_BLK)finishBlk {
    ...
    if (_imgView) {
        _imgView.image = _tailorImage; //viewDidLoad 在释放前不会再执行configImageView, 再次设置_imgView.image以避免一直显示第一张图片
    }
    
    if (form == JITailorFormSquare){
        _slX = 0;//  SquareDistance; //
        _slY = ScreenHeight/2 - ScreenWidth/2;//  ScreenHeight/2 - SquareHeight/2; //
        _slW = ScreenWidth;//  SquareWidth;  //
        _slH = ScreenWidth;//  SquareHeight; //
    }
    
    if (form == JITailorFormRectangle){
        CGFloat ra = 292.5f/375.0f; //高/宽比例
        _slW = ScreenWidth;
        _slH = _slW * ra;
        _slX = 0;
        _slY = ScreenHeight/2 - _slH/2;
    }
    
    if (form == JITailorFormCoterie){
        _slX = 0;
        _slY = ScreenHeight/2 - ScreenWidth/2;
        _slW = ScreenWidth;
        _slH = ScreenWidth;
    }
}
...
- (void)viewDidLoad {
    ...
    if (_tailorForm == JITailorFormCoterie) {
...
- (void)configImageView {
    ...
    //_imgView.layer.position = CGPointMake(0, ScreenHeight/2 - ScreenWidth/2); //设置位置
...
- (void)panAction:(UIPanGestureRecognizer *)pan {
    //selecView位置, 常量
    CGFloat sX = _slX; // 0;
    CGFloat sY = _slY; // ScreenHeight/2 - ScreenWidth/2;
    CGFloat sW = _slW; // ScreenWidth;
    CGFloat sH = _slH; // ScreenWidth;
    ...
    //上边距限制
    if (endY >= sY) {
//        _imgView.transform = CGAffineTransformMakeTranslation(endX, sY);
        _imgView.frame = CGRectMake(endX, sY, endW, endH);
    }
    //左
    if (endX > 0)  {
//        _imgView.transform = CGAffineTransformMakeTranslation(0, endY);
        _imgView.frame = CGRectMake(0, endY, endW, endH);
    }
    //下
    if ((endY + endH) < (sY + sH)) {
//        _imgView.transform = CGAffineTransformMakeTranslation(endX, sY+sH-endH);
        _imgView.frame = CGRectMake(endX, sY+sH-endH, endW, endH);
    }
    //右
    if ( (endX+endW) < (sX+sW) )  {
//        _imgView.transform = CGAffineTransformMakeTranslation(sX+sW-endW, endY);
...
-(void)pinchAction:(UIPinchGestureRecognizer *)pinch
{
    ...
    //JJLog(@"pinch_X:%f  pinch_Y:%f  pinch_W:%f  pinch_H:%f",endX,endY,endW,endH);
...
- (void)configShadeView
{
    ...
    downView.frame = CGRectMake(0, _slY + _slH, _slW, ScreenHeight - _slY - _slY);
...
//    _circleOriginPoint = CGPointMake(self.view.center.x-115, self.view.center.y-115);
    
    lineLayer.path = mutablePath;
    //    NSArray* array = [[NSArray alloc] initWithObjects:[NSNumber numberWithInt:4],[NSNumber numberWithInt:0], nil];
    
    lineLayer.lineDashPhase = 1.0;
    //    lineLayer.lineDashPattern = array;
...
- (void)configSlecView
{
    ...
    //九宫格分割线
    if (_tailorForm == JITailorFormSquare) {
...
- (void)configBackButton
{
    ...
    label.frame = CGRectMake(ScreenWidth/2-100/2, 24.5, 100, 14);
...
- (void)configButton
{
    ...
    cancelBtn.frame = CGRectMake(0, ScreenHeight-48-HomebarHeight, ScreenWidth/2, 48);
    cancelBtn.backgroundColor = UIColorRGBA(180, 180, 180, 1);
    [cancelBtn addTarget:self action:@selector(cancelBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [cancelBtn setTitle:@"取消" forState:UIControlStateNormal];
    [self.view addSubview:cancelBtn];
    
    UIButton  * sureBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    sureBtn.frame = CGRectMake(ScreenWidth/2, ScreenHeight-48-HomebarHeight, ScreenWidth/2, 48);
    sureBtn.backgroundColor = UIColorRGBA(86, 86, 86, 1);
...
- (void)sureBtnAction:(UIButton*)button
{
    ...
    _rectX = (_slX - _imgView.frame.origin.x) * _rate / a; //max_multiple / a
    _rectY = (_slY - _imgView.frame.origin.y) * _rate / a; //max_multiple / a
    _rectW = _slW * _rate / a;
    _rectH = _slH * _rate / a;
    ...
    if (_tailorForm == JITailorFormRectangle){
        UIImage* ouImg = [[JJTailorManager manager] tailorImage:_imgView.image rect:CGRectMake(_rectX, _rectY, _rectW, _rectH)];
        _finishBlk(ouImg);
    }
