...
- (void)viewDidLoad {
    ...
    [super viewDidLoad];
    [self setupSubviews];
    [self bindViewModelWithViewWillAppear:NO];
}
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self bindViewModelWithViewWillAppear:YES];
...
- (void)bindViewModelWithViewWillAppear:(BOOL)isViewWillAppear{
    ...
    [_viewModel getMyWalletInfo:isViewWillAppear];
    ...
    switch (indexPath.section) {
        case 1:
        {
            if ([self.viewModel.balance integerValue] <= 0) {
                ...
                [JRToast showWithText:@"您目前没有余额可以提现"];
//                return;
            }
